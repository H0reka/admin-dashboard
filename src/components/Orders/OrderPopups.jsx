...
export const DiscountPopup = ({ setPopUp, popUp, order}) => {
  ...
  const handleSubmit = () => {
    const payload = {...order, discount: Number(discountPercent), orderId: order.id}
    console.log(payload);
    //API call to update order (Discount)
    // axios.put(`/api/orders`, payload, {
    //   headers:{
      ...
      {popUp && (
        <>
          ...
          <div className="absolute flex flex-col gap-4 z-40 top-[20%] left-[45%] bg-indigo-600/50 backdrop-blur-lg p-3 rounded-lg">
            ...
            <p><span className="font-bold">Item Total: </span><span>{total}</span></p>
            <InputField label="Discount %" onChange={(e) => handleDiscountPercentChange(e)}  value={discountPercent}/>
            <InputField label="Discount Amount" onChange={(e) => handleDiscountAmountChange(e)} value={discountAmount}/>
            <p><span className="font-bold">New Total: </span><span>{total - discountAmount}</span></p>
            <input type="button" className="p-3 bg-white rounded-lg cursor-pointer text-black" value="Submit" onClick={handleSubmit}/>
          </div>
        </>
      )}
    </>
...
export const RiderPopup = ({popUp, setPopUp, orderId})=>{
  ...
  const handleSubmit = async () =>{
    ...
    console.log(payload);
    try{
      await axios.put('/api/rider/order/assign', payload, {headers:{Authorization: `Bearer ${token}`}});
      ...
      {popUp && (
        <>
          ...
          <div className="absolute flex flex-col gap-4 z-40 top-[20%] left-[45%] bg-indigo-600/50 backdrop-blur-lg p-3 rounded-lg">
            ...
            <select onChange={(e)=>setRider(e.target.value)} className="text-white bg-slate-800 rounded-lg p-3" defaultValue="Select Rider">
              <option disabled>Select Rider</option>
              {riderList.map((rider, index)=>(
                <option key={index} value={rider.id}>{rider.name} {rider.phoneNum}</option>
              ))}
            </select>
            <input type="button" className="p-3 bg-white rounded-lg cursor-pointer text-black" value="Submit" onClick={handleSubmit}/>
          </div>
      ...
      {popUp && (
        <>
          ...
          <div className="absolute flex flex-col gap-4 z-40 top-[20%] left-[45%] bg-indigo-600/50 backdrop-blur-lg p-3 rounded-lg">
            ...
            <InputField label="Delivered" onChange={(e)=>setDelivered(e.target.value)} value={delivered}/>
            <InputField label="Cancelled" onChange={(e)=>setCancelled(e.target.value)} value={cancelled}/>
            <input type="button" className="p-3 bg-white rounded-lg cursor-pointer text-black" value="Submit" onClick={handleSubmit}/>
          </div>
        </>
      )}
