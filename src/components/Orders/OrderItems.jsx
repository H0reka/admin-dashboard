  ...
  const OrderItems = (props) => {
    ...
    const columns = [
      ...
      columnHelper.accessor("", {
        cell: ({row})=>(<button onClick={()=>setPopup(true)}><FaEdit className="cursor-pointer" size={24}/></button>),
        header: "Action",
      }),
    ];
    ...
    const checkCompleted = (row)=>{
      if(row.deliveredQuantity + row.cancelledQuantity == row.quantity) return true;
      return false;
    }
    return (
      <div className="p-2 max-w-5xl mx-auto text-white fill-gray-400 ">
        {popUp && <QuantityPopup popUp={popUp} setPopUp={setPopup} order={props.data}/>}
        {/* Table */}
              ...
              {table.getRowModel().rows.length ? (
                table.getRowModel().rows.map((row, i) => (
                  <tr
                    key={row.id}
                    className={`
                    ${checkCompleted(row.original) ?  "bg-green-600/50":"bg-red-600/50" }
                    `}
                  >
