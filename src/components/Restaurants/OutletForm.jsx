...
const OutletForm = ({ initialData, restaurantId }) => {
  const {
    control,
    handleSubmit,
    formState: { errors },
    ...
    defaultValues: {
      ...
      kitchenStartTime: initialData?.kitchenStartTime.substring(11, 16) || "",
      paymentWindow: initialData?.paymentWindow || "PW_0_1",
      wallet: {
        creditLimit: initialData?.wallet.creditLimit || 0,
        walletBalance: initialData?.wallet.walletBalance || 0,
      } || { creditLimit: 0, walletBalance: 0 },
    },
  });
  const [POD, setPOD] = useState(initialData?.podAvailable);
  ...
  const onSubmit = async (data) => {
    if (initialData) {
      ...
      try {
        ...
        await axios.put(
          `${server}/admin/outlet/${initialData.id}/paymentondelivery?status=${
            POD ? "ENABLE" : "DISABLE"
          }`,
          {},
          { headers: { Authorization: `Bearer ${token}` } }
    ...
    >
      <div className="text-black ">
        ...
        >
          <div className="grid md:grid-cols-12 col-span-12 gap-3 md:p-28 justify-between items-center">
            ...
            <div className="col-span-6 flex flex-row items-center justify-left gap-2">
              ...
              <input
                ...
                defaultValue={initialData?.kitchenStartTime.substring(11, 16)}
                {...register("kitchenStartTime")}
              />
            </div>

            <NumberInput name="longitude" label="Longitude" context="outlet" />
                ...
                <div className="text-white flex flex-row gap-8 items-center col-span-6">
                  ...
                  <label className="flex items-center cursor-pointer col-span-6  gap-2">
                    ...
                    <input
                      ...
                      defaultChecked={initialData.podAvailable}
                ...
                <div className="text-white flex flex-row justify-between items-center col-span-6">
                  <p>
                    ...
                    <span className="font-semibold text-lg">
                      {initialData?.wallet.walletBalance}
                    </span>
                  </p>
                  {initialData?.wallet.walletBalance !== 0 ? (
                    <input
                      type="button"
          ...
          <BalancePopup
            id={initialData?.id}
            walletBalance={initialData?.wallet.walletBalance}
            creditLimit={initialData?.wallet.creditLimit}
          />
        ...
        {initialData ? <TransacTable id={initialData?.id} /> : ""}
      </div>
    </OutletContext.Provider>
  );
};
