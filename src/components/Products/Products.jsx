...
const Products = () => {
  ...
  useEffect(() => {
    if(!search)
    {
      const apiURL = category==0?"api/products":`noadminapi/products/categories/${category}`;
      axios
      .get(`${apiURL}?page=${page-1}&sortBy=${column}&sortDirection=${order}`, {
        headers: {
      ...
      axios
      .get(`/noadminapi/products/search?keywords=${search}&page=${page-1}&sortBy=${column}&sortDirection=${order}`)
      .then((res) => {
        const data = res.data.content;
        setmaxPages(res.data.totalPages);
        setProductList(data);
