...
const AddUser = () => {
    ...
    const onSubmit = (data) =>{
        ...
        try{
          axios.put('/api/user', data, {headers:{Authorization: `Bearer ${token}`}});
        navigate('/users');
  ...
  return (
    <div className="ml-[4.2rem] lg:ml-[10.3rem] p-2">
      ...
      <form action="" onSubmit={handleSubmit(onSubmit)} className='flex flex-col justify-items-start items-center gap-2'>
        <Controller
            name="phoneNum"
            control={control}
            render={({field})=>{
