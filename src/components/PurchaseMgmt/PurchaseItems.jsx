...
import { useState } from "react";
  ...
  const PurchaseItems = (props) => {
    ...
    const columns = [
      ...
      columnHelper.accessor("", {
        cell: ({row}) => (
          <select defaultValue={row.original.transaction.transactionStatus} className="text-white dark:bg-neutral-900 dark:border-neutral-700 rounded-lg py-3 px-2.5" onChange={(e)=>handleStatusChange(row.original.id,e.target.value)}>
